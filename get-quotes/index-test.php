...
?>
	...
	<div class="get-info container style-1">
		...
		<div id="form">
			<form id="forms" enctype="application/json" method="POST">
				...
				<input type="hidden" name="Sub_ID" value="<?php if($_SESSION['SubSub_ID']) { echo $_SESSION['SubSub_ID']; } else { echo substr(htmlentities($_GET['keyword']),0,100); } ?>">
				<?php if($_SESSION['SubSub_ID']) { ?>
					<input type="hidden" name="ad_id" value="<?php echo $_SESSION['HIT_ID']; ?>">
				<?php } ?>
        ...
        <input type="hidden" name="Age" id="age" value="<?php if(isset($_GET['Age'])) { echo $_GET['Age']; }?>" />'
              ...
              <div class="row">
								<label class="col-xs-3 nopadding">DOB</label>
								<div class="form-group col-xs-9 nopadding">
									<input id="dob" id="dob" name="DOB" type="tel" autocomplete="bday" maxlength="10" minlength="10" class="form-control" placeholder="mm/dd/yyyy" required <?php if(isset($_GET['First_Name'])) { echo 'value="'.$_GET['DOB'].'"'; }?>>
								</div>
							</div>
							<div class="row">
    ...
    $('#dob').mask("99/99/9999");
		...
		$('#age').on('change', function() {
			if ($('#src').val() != 'Infinix-KFB' && $('#src').val() != 'Magenta') {
				if ($(this).val() >= 65 ) {
					$('#type').val('23');
					$('#src').val('WebPostM');
				} else {
